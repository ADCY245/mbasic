...
function addBlanketToCart() {
  const product = {
    id: 'blanket_' + new Date().getTime(),  // unique ID
    name: 'Blanket - ' + document.getElementById('blanketSelect').value,
    quantity: parseInt(document.getElementById('quantityInput').value),
    total_price: parseFloat(document.getElementById('finalPrice').textContent.replace('₹', '')) || 0,
    redirect_route: 'blankets'  // your Flask route name
  };
  ...
  .then(res => res.json())
  .then(data => {
    if (data.success) alert('Blanket added to cart!');
  });
}
